...
then
  echo "Error connecting to the ftp to get the static data"
  exit 1
fi
set -e
for country in ${GTFS_LCL_DIR}/* ; do
  ...
  for city in ${GTFS_LCL_DIR}/${countryShort}/* ; do
    ...
    echo "Extract data"
    tar -xf ${GTFS_LCL_DIR}/${countryShort}/${cityShort}/gtfs.tar.gz -C ${GTFS_LCL_DIR}/${countryShort}/${cityShort} || true
    echo "Import ${DB_NM}"
